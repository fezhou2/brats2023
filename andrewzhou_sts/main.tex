...
\usepackage{caption}
\usepackage{fancyhdr}
\usepackage{float} % Add this to your preamble if not already present
\onehalfspacing
\setlength{\parindent}{0.5in}
\usepackage{parskip}
\setlength{\parindent}{0pt}
% Custom footer for page numbers (bottom right, starting after abstract)
\pagestyle{fancy}
\fancyhf{}
\fancyfoot[R]{\thepage}
...
\centering
\vspace*{2in}
{\LARGE\bfseries Enhancing Neuro-Oncology Through Self-Assessing Deep Learning Models for Brain Tumor Unified Model for MRI Segmentation\par}
\vspace{2in}
...
Equally important is the need for anatomically aware segmentation. Current tumor segmentation models often operate in isolation from surrounding brain structures, treating the tumor as a localized object rather than part of an interconnected anatomy. In neurosurgical planning, understanding the spatial relationship between tumors and critical functional regions is essential for preserving patient function. Whole brain segmentation enables surgeons to map critical structures and avoid damage during tumor resections and other interventions \cite{desikan2006}. Current methodologies address brain tumor segmentation tasks and whole brain segmentation tasks separately, leading to inefficiencies and a lack of integration between them. Furthermore, most deep learning models are trained and validated on benchmark datasets such as BraTS, which represent only a narrow subset of scanner types, institutions, and patient populations. In practice, brain MRIs vary widely in acquisition protocols, magnetic field strengths, voxel resolutions, and patient demographics. As a result, models trained on one dataset often fail when exposed to new domains. Achieving robustness across multiple datasets, scanners, and brain types is critical for real-world usability. Future models must therefore be designed to handle heterogeneous data distributions and maintain consistent performance across diverse clinical settings.
...
On the other hand, extensive exploration has been conducted on brain tumor segmentation tasks with the use of various deep neural networks. Deep neural networks have emerged as a favored method in recent years, as seen by their application in a majority of works on the BraTS dataset. The U-Net architecture \cite{ronneberger2015} was introduced by Ronneberger et al.\ in 2015 and has since become widely used due to providing accurate segmentation masks with limited training data \cite{zhou2024metastasis}. This has made it especially popular in biomedical imaging segmentation as many of these fields experience a lack of data, of which include brain tumor segmentation. However, the original U-Net struggles with handling class imbalance and segmenting objects of varying sizes effectively. A wide variety of other U-Net-based methods have been proposed \cite{zheng2022,hechri2024}.
In recent years, studies have also presented various models in conjunction such as DeepMedic \cite{kamnitsas2016}, and FCN \cite{long2015}. In parallel, whole brain segmentation remains a challenging problem due to the large number of labels compared to the limited amount of available training data \cite{fischl2012}. Manual segmentation of the whole brain remains a difficult and tedious task, which prevents the production of large labeled datasets.

Traditional techniques for brain MRI segmentation, such as FreeSurfer \cite{fischl2012} and BrainSuite \cite{shattuck2002}, involve atlas-based methods and statistical models. These methods are computationally expensive and require several hours to process a single image. These limitations restrict scalability to larger studies with thousands of images or to clinical workflows where immediate results are required. As such, deep learning methods have been applied for whole brain segmentation \cite{yu2022,coupe2020}. Earlier methods were constrained to only 2D frameworks and patch-wise approaches due to memory constraints. However, these approaches often suffered from discontinuities between patches. Several studies have also prioritized coarse segmentation, analyzing fewer than 35 structures which reduces computational demands and improves processing speed. However, with more than 100 segmentation classes, whole brain segmentation becomes more complex. Due to lack of training data for the large number of classes, only a handful of works have addressed this problem regime \cite{xiong2019,henschel2020}. Furthermore, most of the proposed methods for fine-grained segmentation are based on patch-wise strategies or 2D segmentation. More recent developments, such as SLANT \cite{xiong2019} and FastSurfer \cite{henschel2020}, have utilized 3D CNN-based approaches that divide the input volumes into smaller subregions.
...
\subsection{Dataset}
This study employs two datasets: the Brain Tumor Segmentation (BraTS) 2023 dataset \cite{BraTS2023} and the Open Access Series of Imaging Studies (OASIS-1) \cite{marcus2010}. The BraTS 2023 dataset includes 1,248 patients with four MRI modalities: T1c, T1n, T2f, and T2w, and labels tumor regions as necrotic (NCR), peritumoral edema (ED), or enhancing tumor (ET). OASIS-1 is a publicly available neuroimaging dataset that includes T1-weighted MRI scans from 425 subjects, ranging from 10 to 96 years old. It is preprocessed with FreeSurfer, which provides 133 whole brain classes with both subcortical and cortical regions. The BraTS2023 dataset is used for training both CMODEL and UMODEL, while the OASIS-1 dataset is used for UMODEL training only.

While OASIS-1 includes labels for 48 subcortical structures, this study reduces the number of labels to achieve better segmentation when there is no need to distinguish between certain structures. Some structures are functionally identical but are labeled separately for left and right brain hemispheres, so these pairs were combined into single labels. Other minor structures (such as hyperintensities) were also grouped into single labels, resulting in a total of 18 subcortical structures. Similarly, the 68 cortical structures in OASIS-1 were reduced to 34 cortical structures, resulting in a total of 52 whole brain structures across both datasets. The three tumor masks provided by BraTS2023 were combined into a single mask and represented by a single additional tumor label in OASIS-1.

To generate whole brain segmentation labels, most datasets (including OASIS-1) use FreeSurfer \cite{fischl2012}. Unfortunately, it is computationally intensive, requiring 5--10 hours to process a single image, making it unsuitable for processing over 1,200 MRIs in BraTS2023. In recent years, FastSurfer has been reported to perform whole brain segmentation with high accuracy and significantly shorter processing time. For this study, all images in the BraTS2023 dataset were preprocessed with FastSurfer and the labels were converted into the same 52 whole brain structures used for the OASIS-1 dataset. The combined dataset contained both healthy and cancerous MRI images with segmented labels of tumor and healthy brain regions and was used for UMODEL training.

For CMODEL training, four modalities of MRI images were used from the BraTS2023 dataset. For UMODEL training, only the T1 modality was used as this is the only modality available in the OASIS-1 dataset.
...
\end{equation}
is the Pearson correlation between uncertainty target and true error. The weight hyperparameters were set to $\lambda_{RMSD} = 0.5$ and $\lambda_{corr} = 0.05$.
...
\subsection{Unified Model Prediction Visualization}
Figure~\ref{fig:umodel_seg_view} compares visualized segmentations from FreeSurfer, FastSurfer, and UMODEL (UM2 used). The images are generated with 3D Slicer software \cite{pieper2004}. Figures mentioned in the text compare segmentation performances of UM2, FastSurfer, and FreeSurfer for cancerous brains. In the UM2 segmentation, both whole brain structures and predicted tumor masks T(P) are shown. Since FreeSurfer and FastSurfer only predict whole brain structures, original tumor masks T(O) are overlaid to provide meaningful comparisons.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{img/umodel_seg_view.png}
    \caption{Visual comparison of UM2 segmentation results with FreeSurfer and FastSurfer outputs. Images generated with 3D Slicer software \cite{pieper2004}.}
    \label{fig:umodel_seg_view}
\end{figure}
